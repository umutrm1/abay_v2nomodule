...
const DialogMusteriMusteriSec = ({ projelerArray, dialogTriggerClassName, dialogTriggerTitle, setSeciliProje,setSeciliBayi }) => {
    const [searchTerm, setSearchTerm] = useState("");
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 5;
    const filteredProjeler = projelerArray.filter(proje =>
        proje.musteri_isim.toLowerCase().includes(searchTerm.toLowerCase())
    );
...
const ProjelerTable = ({ projelerArray, setSeciliProje,setSeciliBayi, searchTerm, setSearchTerm }) => {
    return (
                    ...
                    {projelerArray.map(proje => {
                        return (
                            <tr key={proje.id} className="flew flew-row">
                                <th className="w-2/3 text-xl">{proje.musteri_isim}</th>
...
export default DialogMusteriMusteriSec;