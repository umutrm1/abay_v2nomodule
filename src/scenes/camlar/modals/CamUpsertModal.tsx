// Path: @/scenes/camlar/modals/CamUpsertModal.tsx
...
export default function CamUpsertModal({
  open,
  onOpenChange,
  mode,
  cam,
  ...
  const [form, setForm] = useState<CamForm>({
    cam_isim: "",
    thickness_mm: 1,
    belirtec_1: 0,
    belirtec_2: 0,
  ...
  return (
    <ModalShell
      ...
      title={mode === "edit" ? `Cam Düzenle${cam?.cam_isim ? `: ${cam.cam_isim}` : ""}` : "Yeni Cam Ekle"}
      size="md"
      contentClassName="w-[94vw] max-w-md"
      footer={
        <div className="flex justify-end gap-3">
          <AppButton variant="kurumsalmavi" type="button" onClick={() => onOpenChange(false)} disabled={submitting}>
            Vazgeç
          </AppButton>
    ...
    >
      <div className="grid gap-4 py-4">
        ...
        <input
          ...
          name="cam_isim"
          value={form.cam_isim}
          onChange={(e) => setForm((p) => ({ ...p, cam_isim: e.target.value }))}
