// ==============================
// File: src/scenes/setpassword/SetPassword.jsx
// Açıklama: Yeni bayinin e-posta ile aldığı token ile şifre belirlediği ekran.
// Tasarım: Minimal, modern kart – responsive – Tailwind.
// ==============================
...
export default function SetPasswordPage() {
  ...
  // Basit kurallar – backend zorunluluklarını bilmiyorsak güvenli bir minimum uyguluyoruz
  ...
  useEffect(() => {
    // token yoksa login'e yönlendir veya hata göster
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErr(err.message || "Bilinmeyen bir hata oluştu.");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-6">
        <div className="w-full max-w-md bg-white border border-gray-200 rounded-2xl shadow-sm p-8 text-center">
          ...
          <button onClick={() => navigate("/login")} className="btn bg-blue-600 hover:bg-blue-700 text-white w-full">Girişe Dön</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center  from-indigo-50 via-white to-blue-50 p-6">
      <div className="w-full max-w-md bg-white/90 backdrop-blur border border-gray-200 rounded-2xl shadow-sm p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative mt-1">
              <input
                ...
                required
              />
              <button type="button" onClick={() => setShowPw(s => !s)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700">
                {showPw ? (
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.86-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.5a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.243 4.243L9.88 9.88" />
                  </svg>
                ) : (
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
          ...
          <button
            type="submit"
            disabled={!canSubmit}
            className={`btn w-full ${canSubmit ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-gray-200 text-gray-500 cursor-not-allowed"}`}
          >
