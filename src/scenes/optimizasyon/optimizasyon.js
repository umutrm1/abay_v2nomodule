...
// Path Alias: optimizasyon/optimizasyonYap
...
const optimizasyonYap = (siparis, OptimizasyonSonuclar = []) => {
  ...
  // Son fire eşiği (mm) – gerektiğinde buradan elle değiştirilebilir
  const acceptableFire = 0;
  ...
  grupMap.forEach(({ profilId, profilIsim, stockLength, pieces }) => {
    ...
    function findBestCombination(piecesArr, stockLen, fireTol) {
      ...
      for (let i = 0; i < piecesArr.length; i++) {
        ...
        if (lastAcceptableWaste < acceptableFire) {
          lastAcceptableCombo = [];
        ...
        for (let j = 0; j < piecesArr.length; j++) {
          ...
          if (total + piecesArr[j] <= stockLen) {
            ...
            if (interimWaste >= acceptableFire && interimWaste < lastAcceptableWaste) {
        ...
        // Nihai waste < acceptableFire ise "bir önceki en iyi ara" kombinasyonu kabul et
        if (waste < acceptableFire && lastAcceptableCombo.length) {
